...
def main():
    ...
    parser.add_argument("--validation-targets", action="store_true", help="Print current fingerprints for a new check; never stamps a validation")
    ...
    try:
        if args.new:
            ...
            if (args.guide or args.validation_targets) and args.format != "json":
                parser.error("--guide returns JSON; use --format separately for a readable report")
            if args.guide and args.validation_targets:
                parser.error("Choose --guide or --validation-targets, not both")
            assessment = Assessment.model_validate_json(args.input.read_text())
            result = validation_targets(assessment) if args.validation_targets else guided_review(assessment) if args.guide else assess(assessment)
            if args.format != "json":
