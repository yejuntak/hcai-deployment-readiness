...
The previous text repeatedly used short slogans and contrast formulas. The revision replaces those patterns with explanations of the task, evidence and next action. It also reduces repeated warnings where the same boundary can be stated clearly once within a section. Necessary warnings remain in each document that may be read on its own.

| Area | Editorial change | Preserved content |
| --- | --- | --- |
| Website and starting guide | Explain who can use the review, what to bring and what it produces. Reduce promotional headings. | Engineering-only scope, voluntary participation and evidence limits. |
| Protocol, QUICK-6 and full profile | Use direct instructions and clearer transitions between requirements. | Six gates, risk thresholds, required fields, formulas and stop rules. |
| Criteria and scenarios | Clarify checks and examples without altering their obligations. | Fifteen criterion IDs, gate mappings and constructed-example labels. |
| MCP, reports and Skill | Ask concrete questions and explain results without compressed slogans. | Tool names, data keys, deterministic decisions, privacy and authority limits. |
| Research and audit documents | Separate findings, interpretation and proposed evaluation more explicitly. | Sources, causal limits, untested assumptions and the distinction between study and practice. |

One cost paragraph could be read as subtracting residual manual work twice. It now follows the existing full-profile formula explicitly: gross savings already deduct residual manual work; net savings also deduct review, correction, escalation and rework. The formulas and engine have not changed.
