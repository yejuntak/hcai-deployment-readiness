...
Protocol 0.1-rc.4-candidate.2; candidate criteria, not certification.
...
Each important artifact and each requirement-artifact pair has an executed walkthrough or test. The check identifies the exact tested artifact digest.
...
Represent normal, edge and recovery paths with triggers, behavior, resulting state, data treatment, owner and requirement links; review dependencies.
...
Classify all six risk dimensions with rationale. Use the highest dimension; moderate, high or unknown risk cannot qualify through QUICK6.
...
Review reference defects, findings and unresolved risks. Unresolved critical findings cannot be waived. Add independent review and deeper plans when the risk tier requires them.
