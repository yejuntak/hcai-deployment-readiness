...
ZIP_NAME = 'HCAI-v0.1-rc.4-candidate.2.zip'
...
def candidates():
    ...
    frozen = json.loads((ROOT / 'historical/candidate-1-manifest.json').read_text())
...
def main():
    ...
    wheel = ROOT / 'dist/hcai_readiness_mcp-0.2.0rc2-py3-none-any.whl'
    if not wheel.exists():
    ...
    skill_zip = RELEASE / 'ai-ready-0.2.0-rc.2.zip'
    zip_files(skill_zip, sorted(skill_files), ROOT / 'skills')
    files = candidates()
    checksum_lines = ['# Candidate 0.1-rc.4-candidate.2; original rc.3 checksums are frozen under historical/.']
    ...
    zip_files(RELEASE / ZIP_NAME, files + [ROOT / 'SHA256SUMS', RELEASE / wheel.name, skill_zip], ROOT,
              prefix='HCAI-v0.1-rc.4-candidate.2/')
    published = [RELEASE / ZIP_NAME, skill_zip, RELEASE / wheel.name, *(ROOT / 'output/pdf').glob('*v0.1-rc.4-candidate.2.pdf')]
    manifest = {'protocol_version': '0.1-rc.4-candidate.2', 'status': 'candidate', 'files': {
    ...
    if args.site_root:
        ...
        target = site_root / 'static/research/ai-readiness/rc4-candidate-2'
