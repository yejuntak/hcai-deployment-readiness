...
ZIP_NAME = 'HCAI-v0.1-rc.4-candidate.3.zip'
...
def candidates():
    ...
    for manifest in ('candidate-1-manifest.json', 'candidate-2-manifest.json'):
...
def main():
    ...
    wheel = ROOT / 'dist/hcai_readiness_mcp-0.2.0rc3-py3-none-any.whl'
    if not wheel.exists():
    ...
    skill_zip = RELEASE / 'ai-ready-0.2.0-rc.3.zip'
    zip_files(skill_zip, sorted(skill_files), ROOT / 'skills')
    files = candidates()
    checksum_lines = ['# Candidate 0.1-rc.4-candidate.3; original rc.3 checksums are frozen under historical/.']
    ...
    zip_files(RELEASE / ZIP_NAME, files + [ROOT / 'SHA256SUMS', RELEASE / wheel.name, skill_zip], ROOT,
              prefix='HCAI-v0.1-rc.4-candidate.3/')
    published = [RELEASE / ZIP_NAME, skill_zip, RELEASE / wheel.name, *(ROOT / 'output/pdf').glob('*v0.1-rc.4-candidate.3.pdf')]
    manifest = {'protocol_version': '0.1-rc.4-candidate.3', 'status': 'candidate', 'files': {
    ...
    if args.site_root:
        ...
        target = site_root / 'static/research/ai-readiness/rc4-candidate-3'
